...
# Use an Australian-localised Faker instance for Brisbane, QLD
fake = Faker('en_AU')
...
business_data = {
    'SaaS': {
        'names': ['NexusFlow', 'DataStream', 'ConnectSphere', 'QuantumLeap AI', 'InnovateCloud', 'SynthWave Analytics'],
        'taglines': ['Integrating Your World', 'The Future of Data, Today', 'Seamless Collaboration', 'AI-Powered Insights'],
    },
    'E-commerce': {
        'names': ['The Local Pantry', 'Outback Gear Co.', 'Coastal Threads', 'Artisan Alley', 'EcoEssentials'],
        'taglines': ['Fresh & Local, Delivered', 'Adventure Awaits', 'Beach-Inspired Fashion', 'Handcrafted Goods'],
    },
    'Consulting': {
        'names': ['Momentum Strategy Group', 'Apex Financial Solutions', 'BlueGum Digital Marketing', 'Ironbark Security'],
        'taglines': ['Driving Growth & Innovation', 'Your Financial Partner', 'Digital Presence, Perfected', 'Securing Your Future'],
    },
    'Manufacturing': {
        'names': ['Precision Components Pty Ltd', 'Sun-Kissed Foods', 'TerraForm Robotics', 'EcoPack Solutions'],
        'taglines': ['Engineered for Excellence', 'Quality Ingredients, Honest Food', 'Automating Tomorrow', 'Sustainable Packaging'],
    }
}

project_names = [
    'Project Phoenix: UI/UX Overhaul', 'API Integration Gateway', 'Predictive Analytics Engine',
    'Natural Language Processing Bot', 'Q4 "Summer Campaign" Launch', 'SEO Content Strategy Revamp',
    'Winter 2026 Collection Launch', 'E-commerce Platform Migration'
]
...
SKILL_LIST = [
    'Python', 'JavaScript', 'React.js', 'Node.js', 'SQL', 'NoSQL', 'MongoDB', 'PostgreSQL',
    'Cloud Computing', 'AWS', 'Azure', 'Google Cloud Platform', 'Docker', 'Kubernetes',
    'CI/CD', 'Git', 'Agile Methodologies', 'Scrum', 'Project Management', 'Product Management',
    'Data Analysis', 'Machine Learning', 'TensorFlow', 'PyTorch', 'Natural Language Processing',
    'Financial Modeling', 'Market Research', 'Business Strategy', 'Venture Capital',
    'Digital Marketing', 'SEO', 'SEM', 'Content Strategy', 'Google Analytics', 'Social Media Management',
    'UI/UX Design', 'Figma', 'Adobe XD', 'User Research', 'Cybersecurity', 'Penetration Testing'
]

# NEW: An expanded list for B2B contexts
BUSINESS_SKILL_LIST = [
    'Strategic Planning', 'Digital Marketing', 'SEO/SEM', 'Content Marketing',
    'Sales Funnel Optimization', 'Lead Generation', 'CRM Management', 'Financial Modeling',
    'Venture Capital Fundraising', 'Operations Management', 'Supply Chain Logistics',
    'Product Management', 'Agile Methodologies', 'User Experience (UX) Design',
    'User Interface (UI) Design', 'Cybersecurity Consulting', 'Cloud Architecture (AWS/Azure/GCP)',
...
class UserFactory(SQLAlchemyModelFactory):
    ...
    contact_email = factory.LazyAttribute(lambda o: f"{o.first_name.lower()}.{
                                          o.last_name.lower()}@example.com")
...
class BusinessSkillFactory(SQLAlchemyModelFactory):
    class Meta:
        ...
        sqlalchemy_session_persistence = 'flush'
        sqlalchemy_get_or_create = ('name',)
    # UPDATED: Use the new, richer list
...
class BusinessTypeFactory(SQLAlchemyModelFactory):
    ...
    name = factory.Iterator(
        ['SaaS', 'E-commerce', 'Consulting', 'Manufacturing'])
...
class IdeaFactory(SQLAlchemyModelFactory):
    class Meta:
        model = models.Idea
        sqlalchemy_session_persistence = 'flush'
    # UPDATED: Removed manual ID generation
...
# NEW: Intelligent partner matching helper
...
def get_complementary_business(session, base_business):
    ...
    # Find a business of the same type (e.g., SaaS) but a different category (e.g., Marketing vs. Finance)
    # and ensure it's not the same business.
    complementary_partner = session.query(models.Business).filter(
...
class BusinessFactory(SQLAlchemyModelFactory):
    ...
    business_type = factory.LazyFunction(
        lambda: get_random(models.BusinessType))
    business_category = factory.LazyFunction(
        lambda: get_random(models.BusinessCategory))

    # UPDATED: Create more realistic personas
    @factory.lazy_attribute
    def phase(self):
        session = UserFactory._meta.sqlalchemy_session
        # Make SaaS businesses more likely to be in Startup/Growth phase
    ...
    @factory.lazy_attribute
    def name(self):
        return random.choice(business_data.get(self.business_type.name, {}).get('names', ['Generic Business Name']))

    @factory.lazy_attribute
    def tagline(self):
        return random.choice(business_data.get(self.business_type.name, {}).get('taglines', ['A Generic Tagline']))

    website = factory.LazyAttribute(
        lambda o: f"https://www.{o.name.replace(' ', '').replace('.', '').lower()}.com.au")
    description = factory.Faker('paragraph', nb_sentences=3)
    address = factory.Faker('street_address')
    city = "Brisbane"  # UPDATED: More general location
...
class ProjectFactory(SQLAlchemyModelFactory):
    ...
    def set_dates(self, create, extracted, **kwargs):
        if self.project_status == 'closed':
            self.project_closed = self.projection_open + \
                timedelta(days=random.randint(30, 180))
        elif self.project_status == 'complete':
            self.project_completion = self.projection_open + \
                timedelta(days=random.randint(60, 365))
...
class ConnectionTypeFactory(SQLAlchemyModelFactory):
    class Meta:
        ...
        sqlalchemy_get_or_create = ('name',)
    name = factory.Iterator(
        ['Partnership', 'Supplier', 'Customer', 'Investor'])
...
class MastermindRoleFactory(SQLAlchemyModelFactory):
    class Meta:
        ...
        sqlalchemy_get_or_create = ('name',)
    name = factory.Iterator(
        ['Leader', 'Strategist', 'Implementer', 'Innovator'])
...
class BusinessConnectionFactory(SQLAlchemyModelFactory):
    ...
    # UPDATED: Use a logical partner-finding approach
    initiating_business = factory.LazyFunction(
        lambda: get_random(models.Business))

    @factory.lazy_attribute
    def receiving_business(self):
        session = UserFactory._meta.sqlalchemy_session
        partner = get_complementary_business(session, self.initiating_business)
        # Fallback to random if no logical partner is found
        return partner or get_random(models.Business, exclude=(models.Business.id == self.initiating_business.id))
...
class UserSkillFactory(SQLAlchemyModelFactory):
    ...
    user = factory.LazyFunction(lambda: get_random(models.User))
    skill = factory.LazyFunction(lambda: get_random(
        models.Skill) or factories.SkillFactory())
...
class ConnectionMastermindRoleFactory(SQLAlchemyModelFactory):
    ...
    connection = factory.LazyFunction(
        lambda: get_random(models.BusinessConnection))
...
# Use an Australian-localised Faker instance for Coopers Plains, QLD
# Current time: Tuesday, August 5, 2025 1:15 PM AEST
fake = Faker('en_AU')
AEST = pytz.timezone('Australia/Brisbane')
...
# ===================================================================
# CORE ENTITY FACTORIES (That don't depend on the helper function)
# ===================================================================
...
# ===================================================================
# HELPER FUNCTION (Now placed after UserFactory is defined)
# ===================================================================

def get_random(model, exclude=None):
    """Helper function to get a random instance of a model from the DB with optional exclusion."""
    session = UserFactory._meta.sqlalchemy_session
    if session and session.query(model).count() > 0:
        query = session.query(model)
        if exclude:
            query = query.filter(not_(exclude))
        return query.order_by(func.random()).first()
...
# ===================================================================
# DEPENDENT FACTORIES (Aligned with your models.py)
# ===================================================================
...
class BusinessFactory(SQLAlchemyModelFactory):
    ...
    business_category = factory.LazyFunction(
        lambda: get_random(models.BusinessCategory))
    phase = factory.LazyFunction(lambda: get_random(models.BusinessPhase))

    @factory.lazy_attribute
    def name(self):
        return random.choice(business_data[self.business_type.name]['names'])

    @factory.lazy_attribute
    def tagline(self):
        return random.choice(business_data[self.business_type.name]['taglines'])
...
# ===================================================================
# LINKING TABLE FACTORIES (These create the relationships/edges)
# ===================================================================
...
class BusinessConnectionFactory(SQLAlchemyModelFactory):
    class Meta:
        ...
        sqlalchemy_get_or_create = (
            'initiating_business', 'receiving_business', 'connection_type')

    initiating_business = factory.LazyFunction(
        lambda: get_random(models.Business))
    receiving_business = factory.LazyFunction(
        lambda: get_random(models.Business))
    connection_type = factory.SubFactory(ConnectionTypeFactory)
    active = True
    date_initiated = factory.Faker(
...
class UserSkillFactory(SQLAlchemyModelFactory):
    class Meta:
        model = models.UserSkill
        sqlalchemy_session_persistence = 'flush'
        # Changed to use object references
        sqlalchemy_get_or_create = ('user', 'skill')
